...
type User struct {
	...
	TwoFASecret  string    `gorm:"size:32" json:"-"`
...
type Wallet struct {
	...
	Status            string    `gorm:"not null;default:active;size:20" json:"status"`
	SecurityLevel     string    `gorm:"not null;default:standard;size:20" json:"security_level"`
...
type Transaction struct {
	...
	GasPrice        string         `gorm:"size:50" json:"gas_price,omitempty"`
...
// Define custom indexes and constraints
func CreateCustomIndexes(db *gorm.DB) error {
	// Composite indexes for better query performance
	
	// Index for user wallet queries
	if err := db.Exec("CREATE INDEX IF NOT EXISTS idx_user_wallet_composite ON wallets(user_id, name, status)").Error; err != nil {
		return err
	}
	
	// Index for transaction queries by user and time
	if err := db.Exec("CREATE INDEX IF NOT EXISTS idx_user_transactions_time ON transactions(user_id, created_at DESC, status)").Error; err != nil {
		return err
	}
	
	// Index for wallet transaction queries
	if err := db.Exec("CREATE INDEX IF NOT EXISTS idx_wallet_transactions_time ON transactions(wallet_id, created_at DESC) WHERE wallet_id IS NOT NULL").Error; err != nil {
		return err
	}
	
	// Index for transaction hash lookups with status
	if err := db.Exec("CREATE INDEX IF NOT EXISTS idx_tx_hash_status ON transactions(tx_hash, status)").Error; err != nil {
		return err
	}
	
	// Index for pending transactions
	if err := db.Exec("CREATE INDEX IF NOT EXISTS idx_pending_transactions ON transactions(status, created_at) WHERE status = 'pending'").Error; err != nil {
		return err
	}
	
	// Index for audit log queries
	if err := db.Exec("CREATE INDEX IF NOT EXISTS idx_audit_user_action ON audit_logs(user_id, action, created_at DESC) WHERE user_id IS NOT NULL").Error; err != nil {
		return err
	}
	
	// Index for session cleanup
	if err := db.Exec("CREATE INDEX IF NOT EXISTS idx_expired_sessions ON sessions(expires_at) WHERE expires_at < NOW()").Error; err != nil {
		return err
	}
